function [mazes, criteria, globalSettings, vr] = MJS_MemoryMaze(vr)
  ...
  %_______________________________________ 1 _____ 2 _____ 3 _____ 4 _____ 5 ____
  mazes     = struct( 'lStart'          , {10    , 10    , 10    , 10    , 10   }   ... final length 300 before turn
                    , 'lCue'            , {15    , 35    , 100   , 100   , 100  }   ...
                    , 'lMemory'         , {5     , 40    , 50    , 200   , 200  }   ...
                    , 'tri_turnHint'    , {true  , true  , true  , true  , true }   ...
                    , 'turnHint_Mem'    , {false , false , false , false , true }   ... turn off visual guide during memory segment                 
                    , 'cueDuration'     , {nan   , nan   , nan   , nan   , inf  }   ... seconds
                    , 'cueVisibleAt'    , {10    , 10    , 10    , 10    , 10   }   ...
                    , 'cueProbability'  , {0     , 0     , 0     , 0     , 0    }   ...
                    , 'cueDensityPerM'  , {0     , 0     , 0     , 0     , 0    }   ...
                    , 'antiFraction'    , {0     , 0     , 0     , 0     , 0    }   ... fraction of trials with inverted reward condition
                    , 'isShaping'       , {false , false , false , false , false}   ... "unconditional love": all choices rewarded, subject to repetition penalty
                    , 'meanChoiceReps'  , {3     , 3     , 3     , 3     , 3   }   ...   for "unconditional love": Mean number of stay trials before reward removed from that side
                    , 'world'           , {1     , 1     , 1     , 1     , 1    }   ... index in vr.worlds
                    );
  criteria  = struct( 'numTrials'       , {25    , 50    , 80    , 100   , 100 }   ... Minimum number of trials the mouse must spend above performance 
                    , 'numTrialsPerMin' , {2     , 2     , 2     , 2     , 2   }   ... Number of trials required per minute to be considered maintaining “good” performance 
                    , 'criteriaNTrials' , {inf   , inf   , 80    , 100   , 100 }     ... Number of trials in the running window used to measure performance for deciding whether to advance to the next maze
                    , 'warmupNTrials'   , {[]    , []    , []    , []    , []  }   ...
                    , 'numSessions'     , {0     , 0     , 0     , 3     , 1000 }   ... Minimum number of sessions the navigator must have above criteria before advancing
                    , 'performance'     , {0     , 0     , 0.6   , 0.9   , 0.9  }   ... Minimum performance criterion to advance maze
                    , 'maxBias'         , {inf   , 0.2   , 0.2   , 0.1   , 0.1  }   ... Max allowed side bias to advance
