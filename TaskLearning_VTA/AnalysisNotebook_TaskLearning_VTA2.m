%Set paths
close all;
experiment = 'mjs_taskLearning_VTA_2'; %If empty, fetch data from all experiments

dirs = addGitRepo('General','TankMouseVR','U19-pipeline-matlab','BrainCogs_mjs');
dirs.data = fullfile(dirs.root,'TaskLearning_VTA2','data');
dirs.results = fullfile(dirs.root,'TaskLearning_VTA2','results',experiment);
dirs.summary = fullfile(dirs.root,'TaskLearning_VTA2','summary',experiment);
dirs.intake = fullfile(dirs.root,'TaskLearning_VTA2','results');
...
if exe.reloadData
    clearvars subjects;
    subjects = struct(...
        'ID',       {"mjs20_201","mjs20_203","mjs20_205"},...
