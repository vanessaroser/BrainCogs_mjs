...
position = (params.positionWindow(1):params.binWidth:params.positionWindow(2)) - 0.5*params.binWidth; 

% Distribute dFF into bins according to Y-position
dFF = cell2mat(cells.dFF');
t = cells.t; %Abbreviate
timeIdx = 1:params.binWidth:params.positionWindow(2)+1; %Downsample positional edges
trialTimes = trialData.time_trajectory(timeIdx,:)'; %Trials x position-bin in cm
