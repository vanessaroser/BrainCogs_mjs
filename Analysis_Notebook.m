...
%Set paths
clearvars;
dirs = addGitRepo('General','TankMouseVR','BrainCogs_mjs');

%Hyperparams
exe = struct('dailyIntakeTable',true,'updateExperData',false); 
...
if exe.dailyIntakeTable
    %data = []; %Set as empty to fetch directly data from bucket
    data = getMouseData(getDirs([]), [], subject);
    intake = dailyIntakeTable(data, getDirs([]), subject);
end
