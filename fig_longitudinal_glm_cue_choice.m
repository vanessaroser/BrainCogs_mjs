function figs = fig_longitudinal_glm_cue_choice_outcome( subjects, vars_cell, glmName, colors )

% vars = struct('pCorrect',false,'pOmit',false,'mean_velocity',false);
for i = 1:numel(vars_cell)
    vars{i} = vars_cell{i};
end
predictorNames = {'towerSide','puffSide','priorRewChoice','priorUnrewChoice','bias'}; %Predictors vs parameters, etc
...
for i = 1:numel(subjects)
    ...
    sessions = ...
        subjects(i).sessions([subjects(i).sessions.taskRule]~="forcedChoice"); %Exclude L-maze data
