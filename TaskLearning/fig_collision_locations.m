...
for i = 1:numel(subjects)
    ...
    for j = 1:numel(subjects(i).sessions)
        ...
        for k = 1:max(mazeIdx)
            ...
            %Do not plot sessions with fewer than 100 included trials
            trialIdx = mazeIdx==k & trials.forward & trials.correct;
            if sum(~isnan(trialData.stuck_time(trialIdx))) < 50
                continue
            end
            ...
            txtX = 0.1*diff(xlim);
            text(txtX,txtY,{'Min dist. from turn: ', [num2str(lCue-max(loc(2,:)),'%3.0f'), ' cm']});
