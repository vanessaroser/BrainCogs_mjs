...
shading = cbrew.blue;
    ...
    if var=="pCorrect_conflict"
        plot(X, mean([data{:}],'omitnan'),'o','MarkerSize',markerSize,...
            'LineStyle','none','LineWidth',lineWidth,'Color',colors.(var));
        plotSEM(X, [data{:}],colors.(var),lineWidth);
        var = "pCorrect";
        data = getData(subjects,var,sensorySessions,altSessions,nSensory,maxSessions); %Co-plot with pCorrect
%         plot(X, [data{:}],'.','MarkerSize',10,'LineStyle','none');
        plot(X, mean([data{:}],'omitnan'),'o','MarkerSize',markerSize,...
            'LineStyle','none','LineWidth',lineWidth,'Color',colors.(var));
        plotSEM(X, [data{:}],colors.(var),lineWidth);
    else
        plot(X, [data{:}],'.','MarkerSize',markerSize,'LineStyle','none');
