...
function [position_mat, maxY, yIndex] = getTrialTrajectories(position, dimString, ySample)
%Select axes index based on input string
idx = {["x","y","theta"],[1,2,3]};
idx = idx{2}(idx{1}==dimString); %Index into position{trial_idx}(:,idx)

%Get positions corresponding to input sample y-positions
maxY            = cellfun(@(pos) cummax(pos(:,2)), position, 'UniformOutput', false);
yIndex          = accumfun(2, @(x) binarySearch(x, ySample, 1, 1)', maxY);
