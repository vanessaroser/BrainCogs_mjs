...
for i = 1:numel(subjects)
    ...
    for j = 1:numel(data_files)
        ...
        hits = struct('all',double(correct),'congruent',double(correct),'conflict',double(correct));
        hits.congruent(~congruent) = NaN;
        hits.conflict(~conflict) = NaN;
        for fields = ["all","congruent","conflict"]
            pCorrect_moving.(fields) = movmean(hits.(fields),[99 0],'omitnan','Endpoints','discard');
            if isempty(pCorrect_moving.(fields))
                pCorrect_moving.(fields) = NaN;
            end
        end
        ...
        sessions(j) = struct(...
            ...
            'maxCorrectMoving', max(pCorrect_moving.all),...
            'maxCorrectMoving_congruent', max(pCorrect_moving.congruent),...
            'maxCorrectMoving_conflict', max(pCorrect_moving.conflict),...     
